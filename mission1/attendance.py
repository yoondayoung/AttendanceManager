...
def update_player(player_data, day) -> list:
    ...
    player_data[0][day_index] += 1
    player_data[1] += add_point
        ...
        for line in f.readlines():
            ...
            if len(attendance) == 2:
                ...
                if player_name not in player_dict:
                    player_dict[player_name] = add_new_player(attendance[1])
                else:
...
def get_player_weekend_attendance(player_data):
    return player_data[0][5] + player_data[0][6]
...
def is_player_removed(player_data):
